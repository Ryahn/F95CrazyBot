...
module.exports = {
    ...
    async execute(interaction) {
        if (!interaction.channel.id === '818944565830221845') {
           return await interaction.reply({ content: 'Please go to a <#818944565830221845>', ephemeral: true });
        }
        if (interaction.channel.nsfw && interaction.channel.id == '818944565830221845') {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(color)
                .setImage(imgUrl)
                .setFooter({ text: "This content is served by an image-search api and cyberpunkxxx is not responsible for any content!" });

            await interaction.editReply({ embeds: [embed] });
        }
        else {
            await interaction.reply({ content: 'Please go to <#818944565830221845> to use this command!', ephemeral: true });
        }
    },
};