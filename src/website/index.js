...
module.exports = client => {
	...
	app.get("/", async (req, res) => {
		return res.json({msg: 'nothing to see yet'})
	...
	app.get("/support", (req, res) => {
		return res.redirect("https://discord.gg/cyberpunkxxx");
	});
	...
	app.get('/idcard', async (req, res) => {
		const canvas = createCanvas(638, 1012);
        const context = canvas.getContext('2d');
		GlobalFonts.registerFromPath('src/website/idcards/fonts/arame-mono-regular', 'arame regular');
		// const idcard = await loadImage('src/website/idcards/id_test.png');
		// context.drawImage(idcard, 0, 0, canvas.width, canvas.height);

        context.font = '20pt "arame regular"';
        context.fillStyle = '#FFFFFF';

        context.fillText('test', 122, 572);
		await loadImage('src/website/idcards/id_test.png').then((data) => {
			context.drawImage(data, 0, 0, canvas.width, canvas.height)
			const imgBuffer = canvas.toBuffer('image/png')
			fs.writeFileSync('src/website/idcards/id_test1.png', imgBuffer)
			res.json({data: canvas.toDataURL()})
		  })
		// context.fillText()
		// res.send('<img src="' + canvas.toDataURL() + '" />')
	})
	app.get('/api/update/founders', async (req, res) => {
		const guild = await client.guilds.cache.get('955312251542319164');
		const gmembers = await guild.members.fetch();
		const members = guild.roles.cache.get('1094091674910728232').members;
		let prevMember = 1;
		const dbClient = client.dbClient;
		const db = dbClient.db("cyberpunkxxx");
		...
		try {
			...
			members.forEach(async (member) => {
				let gMem = guild.members.cache.get(member.id);
				let avatar;
				...
				const result = await collection.findOne(filter1);
				if (!result) {
					const document = {
						"_id": member.user.id,
						...
						"tag": member.user.tag,
						"avatar": avatar,
						"joined": moment(gMem.joinedTimestamp).format('YYYY/MMM/DD'),
						"num": i,
					};
					i++;
	...
	app.get("/founders", async (req, res) => {
		...
		const db = dbClient.db("cyberpunkxxx");
		const collection = db.collection('founders')
		const data = collection.find({}).sort({ num: 1 }).toArray().then(data => {
			return res.render('founders', {data});
				// return product;
			  })
		// console.log(data)
