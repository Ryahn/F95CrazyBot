...
const {cyan, white, greenBright, redBright} = require('chalk');
...
for (const folder of folders) {
    ...
    for (const file of files) {
		...
		if (!command.enable) {
			logger.custom('COMMAND', '#ffffff', '#F5A9F2',`[${cyan(folder)}] ${white(command.data.name)}: ${redBright(command.enable)}`);
		} else {
			logger.custom('LOADER', '#ffffff', '#F5A9F2',`[${cyan(folder)}] ${white(command.data.name)}: ${greenBright(command.enable)}`);
